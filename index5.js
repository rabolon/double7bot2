...
async function run() {
  ...
  if (firstTime) {
    ...
    dataPlot.botProfit = [...dataPlot.prices];
  ...
  // each one tick
    ...
    if (newKline[0][0] > dataPlot.openTime[lastIndex]) {       // shift dataPlot every minute aproximately
      ...
      dataPlot.botProfit.shift();
      dataPlot.botProfit.push(NaN);
    ...
    dataPlot.botProfit[lastIndex] = assetAccu * price + bnbAccu * bnbPrice + baseAccu;
    //console.log(dataPlot.botProfit);

    const now = new Date().getTime();
    const elapsed = now - intiTime;
    
    console.log(`-----------------------------------------------------------------------------------------------------------------------------`);
    console.log(`[${msToTime(elapsed)}] Bot config -> Version: ${version}, Market: ${asset + base}, BB: ${stdDeviations}-${bbandsLength}, Allocation: ${allocation.toFixed(8)} now ${(allocation * price).toFixed(2)}, Max imbalance: ${imbalance}`);
    console.log(`Prices -> ${asset + base} init: ${initPrice.toFixed(2)}, ${asset + base}: ${price.toFixed(2)}, ${bnb + base}: ${bnbPrice.toFixed(2)}, Bot status -> ${status}, Sells: ${assetSellQty}, Buys: ${assetBuyQty}`);
    console.log(`Balance spot -> ${asset}: ${assetBalanceInitial.toFixed(8)}/${assetBalance.toFixed(8)}(${(assetBalance * price).toFixed(2)}), ${base}: ${baseBalanceInitial.toFixed(2)}/${baseBalance.toFixed(2)}, ${bnb}: ${bnbBalanceInitial.toFixed(8)}/${bnbBalance.toFixed(8)}, Profit: ${((assetBalance - assetBalanceInitial) * price + (baseBalance - baseBalanceInitial) + (bnbBalance - bnbBalanceInitial) * bnbPrice).toFixed(2)}`);
    console.log(`Balance bot -> ${asset}: ${assetAccu.toFixed(8)}, ${base}: ${baseAccu.toFixed(2)}, ${bnb}: ${bnbAccu.toFixed(8)}, Fees: ${fees.toFixed(2)}, Profit: ${dataPlot.botProfit[lastIndex].toFixed(2)}`);
  }
}
